...
sql;
    ...
    protected function set_validate_module_stm(){
        ...
        if($this->validate_module_stm === false){
            ...
            return false;
        }elseif(!oci_bind_by_name($this->validate_module_stm, ':acyear', $this->academic_year, 8, SQLT_CHR)){
            $this->report->log_report(2, 'Failed to bind :acyear in set_validate_module_stm()');
            return false;
        }elseif(!oci_bind_by_name($this->validate_module_stm, ':period_code', $this->period_code, 8, SQLT_CHR)){
            $this->report->log_report(2, 'Failed to bind :period_code in set_validate_module_stm()');
            return false;
        }else{
            return true;
        }
...
SELECT SPR_CODE FROM INS_SPR spr
...
WHERE stu.STU_UDF1 = :bucs_id AND sce.sce_ayrc = :ac_year AND sce.SCE_STAC = 'C'
sql;
    }
